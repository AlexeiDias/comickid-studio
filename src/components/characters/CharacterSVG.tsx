...
function HatAcc({ cx, cy, outfitColor }: { cx: number; cy: number; outfitColor: string }) {
  return <><rect x={cx-19} y={cy-22} width="38" height="6" rx="2" fill={outfitColor} stroke="#1a1a2e" strokeWidth="2"/><rect x={cx-13} y={cy-36} width="26" height="15" rx="4" fill={outfitColor} stroke="#1a1a2e" strokeWidth="2"/></>;
}
function CrownAcc({ cx, cy }: { cx: number; cy: number }) {
  return <path d={`M${cx-18},${cy-20} L${cx-13},${cy-32} L${cx-6},${cy-22} L${cx},${cy-34} L${cx+6},${cy-22} L${cx+13},${cy-32} L${cx+18},${cy-20} Z`} fill="#ffd23f" stroke="#1a1a2e" strokeWidth="2"/>;
}
function GlassesAcc({ cx, cy }: { cx: number; cy: number }) {
  return <><circle cx={cx-9} cy={cy} r="7" fill="none" stroke="#1a1a2e" strokeWidth="2"/><circle cx={cx+9} cy={cy} r="7" fill="none" stroke="#1a1a2e" strokeWidth="2"/><line x1={cx-2} y1={cy} x2={cx+2} y2={cy} stroke="#1a1a2e" strokeWidth="2"/><line x1={cx-22} y1={cy-2} x2={cx-16} y2={cy-1} stroke="#1a1a2e" strokeWidth="2"/><line x1={cx+16} y1={cy-1} x2={cx+22} y2={cy-2} stroke="#1a1a2e" strokeWidth="2"/></>;
}
function CapeAcc({ bodyX, bodyY, hairColor }: { bodyX: number; bodyY: number; hairColor: string }) {
  return <path d={`M${bodyX-15},${bodyY} Q${bodyX-25},${bodyY+20} ${bodyX-20},${bodyY+36} Q${bodyX},${bodyY+28} ${bodyX+20},${bodyY+36} Q${bodyX+25},${bodyY+20} ${bodyX+15},${bodyY} Z`} fill={hairColor} stroke="#1a1a2e" strokeWidth="2" opacity="0.85"/>;
}
...
function HumanHead({ cx=40, cy=36, skin='#ffcc99', hair='#333', outfit='#4a90e2', eye='happy', acc='none' }: any) {
  return <g>
    ...
    <Eyes cx={cx} cy={cy+2} style={eye}/><Mouth cx={cx} cy={cy+10}/>
...
function Human({ character, pose, size }: { character: any; pose: CharacterPose; size: number }) {
  const skin=character.skinColor||'#ffcc99', hair=character.hairColor||'#333';
  const outfit=character.outfitColor||'#4a90e2', eye=character.eyeStyle||'happy', acc=character.accessory||'none';
  ...
  // ✅ FIX: Running pose — clear, readable body positions
  if (pose==='running') return (
    <svg width={size} height={size*1.3} viewBox="0 0 80 105" xmlns="http://www.w3.org/2000/svg">
      {acc==='cape'&&<CapeAcc bodyX={42} bodyY={42} hairColor={hair}/>}
      {/* Torso leaning forward slightly */}
      <rect x="26" y="40" width="30" height="26" rx="7" fill={outfit} stroke="#1a1a2e" strokeWidth="2.5" transform="rotate(-8 41 53)"/>
      {/* Left arm swings back (behind body) */}
      <rect x="48" y="44" width="10" height="22" rx="5" fill={outfit} stroke="#1a1a2e" strokeWidth="2" transform="rotate(50 53 44)"/>
      {/* Right arm swings forward */}
      <rect x="18" y="38" width="10" height="22" rx="5" fill={outfit} stroke="#1a1a2e" strokeWidth="2" transform="rotate(-35 23 38)"/>
      {/* Left leg drives forward */}
      <rect x="28" y="64" width="12" height="26" rx="6" fill={outfit} stroke="#1a1a2e" strokeWidth="2" transform="rotate(-22 34 64)"/>
      {/* Right leg kicks back */}
      <rect x="40" y="62" width="12" height="24" rx="6" fill={outfit} stroke="#1a1a2e" strokeWidth="2" transform="rotate(38 46 62)"/>
      <HumanHead cx={44} cy={26} skin={skin} hair={hair} outfit={outfit} eye={eye} acc={acc}/>
    </svg>
  );
  if (pose==='walking') return (
    <svg width={size} height={size*1.35} viewBox="0 0 80 110" xmlns="http://www.w3.org/2000/svg">
      ...
      <ellipse cx="14" cy="60" rx="6" ry="13" fill={outfit} stroke="#1a1a2e" strokeWidth="2" transform="rotate(-20 14 60)"/>
      <ellipse cx="66" cy="64" rx="6" ry="13" fill={outfit} stroke="#1a1a2e" strokeWidth="2" transform="rotate(18 66 64)"/>
...
// ── Animal ───────────────────────────────────────────────────────────────────
function AnimalHead({ species, cx, cy, bodyColor, accentColor, eye, acc, outfit }: any) {
  const renderHead = () => {
    switch (species) {
      case 'cat': return <>
...
function Animal({ character, pose, size }: { character: any; pose: CharacterPose; size: number }) {
  const species=character.species as CharacterSpecies;
  ...
  // ✅ FIX: Animal running — cleaner limb positions
  if(pose==='running') return (
    <svg width={size*1.1} height={size} viewBox="0 0 90 90" xmlns="http://www.w3.org/2000/svg">
      {acc==='cape'&&<CapeAcc bodyX={48} bodyY={36} hairColor={accentColor}/>}
      <rect x="28" y="34" width="36" height="24" rx="9" fill={bodyColor} stroke="#1a1a2e" strokeWidth="2.5" transform="rotate(-10 46 46)"/>
      <Tail tx={62} ty={44}/>
      <Limb x={14} y={34} w={11} h={22} rot={-38} cx={20} cy={34}/>
      <Limb x={58} y={28} w={11} h={22} rot={28} cx={64} cy={28}/>
      <Limb x={20} y={56} w={11} h={22} rot={28} cx={26} cy={56}/>
      <Limb x={54} y={52} w={11} h={22} rot={-22} cx={60} cy={52}/>
      <AnimalHead cx={52} cy={20} {...headProps}/>
    </svg>
  );
  if(pose==='walking') return (
...
function defaultBodyColor(s: CharacterSpecies): string {
  const map: Record<CharacterSpecies,string> = { human:'#ffcc99',cat:'#f4a056',dog:'#c8a068',rabbit:'#e8d8c8',bear:'#8b6340',fox:'#e8621a',frog:'#5aad3c',penguin:'#1a1a2e' };
  return map[s];
}
function defaultAccentColor(s: CharacterSpecies): string {
  const map: Record<CharacterSpecies,string> = { human:'#333',cat:'#fff5e8',dog:'#a07840',rabbit:'#f8c0b8',bear:'#c8a478',fox:'#ffffff',frog:'#78dd58',penguin:'#1a1a2e' };
  return map[s];
}
...
export default function CharacterSVG({ character, pose='standing', size=80 }: Props) {
  ...
  if(!species||species==='human') return <Human character={character} pose={pose} size={size}/>;
