...
const OUTFIT_COLORS= ['#4a90e2','#e74c3c','#2ecc71','#f39c12','#9b59b6','#1abc9c','#e67e22','#34495e','#ff69b4','#00ced1','#ff4500','#8b0000'];
...
const BODY_SHAPES: Array<{key:Character['bodyShape'];label:string}> = [
  {key:'round',label:'⭕ Round'},{key:'square',label:'🟦 Square'},
  {key:'tall',label:'📏 Tall'},{key:'small',label:'🔵 Small'},
];
const EYE_STYLES: Array<{key:Character['eyeStyle'];label:string}> = [
  {key:'happy',label:'😊 Happy'},{key:'cool',label:'😎 Cool'},
  {key:'surprised',label:'😲 Wow'},{key:'sleepy',label:'😴 Sleepy'},
];

// ✅ FIX 3: Extended accessories
    ...
    <div className="fixed inset-0 bg-black/60 flex items-end z-50">
      <div className="bg-amber-50 rounded-t-3xl border-t-4 border-amber-900 w-full"
        ...
        <div className="p-4 pb-10">
          ...
          {/* Outfit color */}
          <Section title="Outfit / Shirt Color">
            <div className="flex gap-2 flex-wrap">
              {OUTFIT_COLORS.map(c=>(
          ...
          {/* ✅ FIX 3: More accessories */}
          <Section title="Accessories & Objects">
            <div className="grid grid-cols-2 gap-2">
              {ACCESSORIES.map(s=>(
                <button key={s.key} onClick={()=>update('accessory',s.key)}
                  className={`btn-ink py-2 px-3 rounded-lg text-sm font-bold text-left ${char.accessory===s.key?'bg-amber-400':'bg-white'}`}>
                  {s.label}
                </button>
              ))}
            </div>
